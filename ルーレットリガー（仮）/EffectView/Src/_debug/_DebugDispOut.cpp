...
void _DebugDispOut::TRACE_S(int Color, std::string str, std::string str2)
{
	SetScreen();
	auto length = GetDrawFormatStringWidth((str + ":%s").c_str(), str2.c_str());
	DrawBox(mesPosX_, mesPosY_ + drawLineY_, mesPosX_ + length, mesPosY_ + drawLineY_ + 16, 0, true);
	DrawFormatString(mesPosX_, mesPosY_ + drawLineY_, Color, (str + ":%s").c_str(), str2.c_str());
	RevScreen();
	drawLineY_ += 16;
}
